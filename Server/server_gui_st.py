...
if "LAMBDA_PARAM_MASK" not in st.session_state:
    st.session_state.LAMBDA_PARAM_MASK = LAMBDA_PARAM_MASK
if "INTERVAL_FRAMES" not in st.session_state:
    st.session_state.INTERVAL_FRAMES = INTERVAL_FRAMES


# Function to generate a random mask
def generate_random_mask(height=32, width=32, sampling_rate=0.5):
    mask = np.random.choice([0, 255], size=(height, width), p=[1 - sampling_rate, sampling_rate])
    return mask
...
def connect_client():
    ...
    try:
        ...
        st.session_state.mask_generator = MaskGenerator(nb_images_mask=128, sample_rate=SAMPLE_RATE, lambda_param=st.session_state.LAMBDA_PARAM_MASK, height=st.session_state.height, width=st.session_state.width)
        ...
        st.session_state.logs += f"Error connecting to client or Initializing Processing: {str(e.__traceback__)}\n"
        st.rerun()
...
def process_frame():
    ...
    try:
        ...
        if st.session_state.frame_count >= st.session_state.INTERVAL_FRAMES:
...
def stop_server():
    ...
    st.session_state.uncompressed_image_count = 0  # Reset uncompressed image count
    st.session_state.data = b""  # Reset data buffer
...
# ✅ **New Input Fields for Lambda & Interval Frames**
st.session_state.LAMBDA_PARAM_MASK = st.sidebar.number_input("Lambda Parameter:", min_value=0.0, max_value=1.0, value=st.session_state.LAMBDA_PARAM_MASK, step=0.05)
st.session_state.INTERVAL_FRAMES = st.sidebar.number_input("Interval Frames:", min_value=1, value=st.session_state.INTERVAL_FRAMES, step=1)
